...
<?php
    ...
    if(isset($_POST['archive_pcp'])){
        $DB = new Database();
        $sql = "INSERT INTO gamingcontest_archive SELECT * FROM gamingcontest";
        $DB->query($sql);
        $sql = "DELETE FROM gamingcontest";
        $DB->query($sql);
        $archiveSuccess = true;
    }
?>
...
<head>
    <title>Gaming Contest Participants</title>
    <meta charset="utf-8">
    <base href="<?= BASE_URL; ?>">
        ...
        <section>
            <div class="container">
                <div class="row">
                    <?php
                        $db = new Database();
                        $sql = "SELECT * FROM gamingcontest ORDER BY RegistrationDate ASC";
                        $result = $db->query($sql);
